...
import NotFoundPage from '@pxp-ui/components/NotFoundPage';
...
const AppRouter = ({
  ...
  const error = `\nPXP: The configured private path ${Pxp.config.privateInitRoute} cannot be found in your current configuration. Or the menu data is empty.\nCheck your configuration.`;
  ...
  // if(Pxp.config.privateInitRoute !== 'first' && !privatePaths.includes(Pxp.config.privateInitRoute)) {
  //   throw error;
  // }
  ...
  return (
    <Router history={history}>
      <div>
        {inMaintenance ? (
          <Switch>
            <Route path="/" render={() => <MaintenanceContainer />} />
          </Switch>
        ) : (
          <Switch>
            <Route
                ...
                >
                  <Switch>
                    {filteredRoutes.map((route) => {
                      ...
                      return (
                        <Route
                          exact={!pages[route.component].noExact}
                          key={key}
                    ...
                    })}
                    {
                      ...
                      filteredRoutes.reduce((subroutes, route) => {
                        if (pages[route.component].detailPages) {
                          pages[route.component].detailPages.forEach((page: any) => {
