...
import { InitValues } from '@pxp-ui/hooks';
...
const DrawForm: FC<any> = forwardRef(({ data, dialog, loading = false }, ref) => {
  ...
  const getSchemaValidation = (byGroup = false, callback) => {
    const getValidations = (nameGroup?: string) => {
      const validations = Object.entries(states)
        ...
        .filter(
          ...
          ([nameKey, val]) => {
            const value: any = val;
            return typeof value.yupValidate === 'object' &&
  ...
  const resetForm = () => {
    ...
    Object.entries(states).forEach(([nameKey, state]) => {
      states[nameKey].reset();
    });
  };
  // hide group by name
  const hideGroup = (nameGroup) => {
    Object.values(states)
      .filter((value: any) => value.group === nameGroup)
      .forEach((field: any) => {
        field.hide();
      });
  };
  ...
  const getDataForSending = (values, callback) => {
    ...
    const thereIsDropZoneArea = Object.entries(states).find(
      ([nameKey, value]) => states[nameKey].type === 'DropzoneArea',
    );
    if (thereIsDropZoneArea) {
      const formData = new FormData();
      Object.entries(values).forEach(([nameKey, val]) => {
        const value: any = val;
        if (states[nameKey]) {
  ...
  Object.entries(states).map(([nameKey, vals], index) => {
    const values: any = vals;
  ...
  return (
    <>
      {data.typeForm === 'normal' &&
        dialog &&
        Object.entries(groupsConfig).map(([nameKey, vals], index) => {
          const continueRenderGroup = Object.values(states).filter(
            (value: any) => value.group === nameKey && !value.isHide,
          );
