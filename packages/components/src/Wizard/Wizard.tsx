import React, { useEffect, useState, useRef, forwardRef, useImperativeHandle, FC } from 'react';
...
const Wizard: FC<any> = forwardRef(({ children, complete, loading = false, orientation = 'horizontal' }, ref) => {
    ...
    <Paper className={classes.root} elevation={6}>
      <div className={ isVertical() ? classes.contentV: classes.contentH }>
        <Stepper activeStep={activeStep} 
          ...
          connector={ !isVertical() ? <ColorlibConnector /> : <div></div> }
        >
          {steps.map((step, i) => (
            <Step key={step.title} 
              className={ clsx( 
