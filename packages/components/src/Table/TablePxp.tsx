...
import { useJsonStore, InitButton } from '@pxp-ui/hooks';
import { setTableState, setPxpTableState } from '@pxp-ui/core/actions/app';
...
function useWidth() {
  ...
  const keys: any = [...theme.breakpoints.keys].reverse();
  return (
...
const TablePxp: FC<any> = forwardRef(({ dataConfig }, ref) => {
  ...
  console.warn('Review the notistack component.');
  const enqueueSnackbar = (msg, opts) => alert(msg);
  const {
    tableName,
    tableLabel,
    idStore,
  ...
  useLayoutEffect(() => {
    if (saveLocalStorage) {
      const currentLocal = localStorage.getItem('routesPxpTable');
      const routesInLocalStorage: any = currentLocal ? JSON.parse(currentLocal) : [];
  ...
  const columnsForDrawing = Object.entries(dataConfig.columns)
    .filter(([, value]) => {
      const val: any = value;
      return val.grid === true || val.grid === undefined;
    })
    .reduce(
      (t, [nameKey, value]) => ({
        ...t,
  ...
  const statePxpTable = useSelector((state: any) =>
  ...
  const jsonStore = useJsonStore(
    statePxpTable ? statePxpTable.getDataTable : dataConfig.getDataTable, Pxp
  );
  const { state, set, data, loading, error } = jsonStore;

  const [dataRows, setDataRows] = useState([]);
  const [total, setTotal] = useState(0); // total is whole total in your query backend
  ...
  const handleCheckInCell = (event, row: any) => {
    const selectedIndex: number = selected.indexOf(row[idStore]);
    let newSelected = [];
  ...
  const handleEdit = (row) => {
    const columnsEdit = Object.entries(dataConfigForEdit.columns).reduce(
      (t, [nameKey]) => {
        const column: any = dataConfigForEdit.columns[nameKey];
        return {
          ...t,
          [nameKey]: {
            ...column,
  ...
  const disableButtonsToolbar = () => {
    Object.values(statesButtonsToolbar).forEach((stateButton: any) => {
      stateButton.disable();
    });
  };
  ...
  const handleChangePage = (event, newPage) => {
    set({
      ...
      params: {
        ...state.params,
        start: rowsPerPage * parseInt(newPage, 10),
      },
    });
    setPage(newPage);
  };
  ...
  const columnsForSearch = Object.entries(dataConfig.columns)
    // eslint-disable-next-line no-unused-vars
    .filter(([nameKey, value]) => {
      dataConfig.columns[nameKey].search === true
    })
    .reduce(
      (t, [nameKey, value]) => {
        const val: any = value;
        return { ...t, [nameKey]: val.filters.pfiltro };
      },
      {},
    );
